/* =====================================================================================
   ...
   HOW TO RUN
   ------------
   1. Connect to the SQL Server instance from your hosting control panel.
   2. Select your own database from the database dropdown (for example: db65922).
      The database is already created for you by the hosting provider.
   3. Run this whole script.

   This script contains no database-level statements and no schema prefix,
   so it always runs inside whichever database you are connected to.

   The script is idempotent: every seed INSERT is guarded by IF NOT EXISTS,
   so running it twice is safe.

   CONTENTS
   --------
   68 tables | 136 foreign keys | 100 check constraints | 57 indexes
    8 triggers | 7 views | 1 stored procedure | 107 permissions | 11 roles

   Phase 2 (not in this file): extended HR, e-invoicing, chart of accounts.
   ===================================================================================== */
...
ALTER TABLE TripTypes
    ADD CONSTRAINT FK_TripTypes_TaxRates FOREIGN KEY (DefaultTaxRateId) REFERENCES TaxRates(TaxRateId);
GO
...
CREATE TABLE Services (
    ...
    CONSTRAINT FK_Services_TaxRates FOREIGN KEY (TaxRateId) REFERENCES TaxRates(TaxRateId)
);
GO
...
CREATE TABLE Customers (
    ...
    CONSTRAINT FK_Customers_PaymentTerms FOREIGN KEY (PaymentTermId) REFERENCES PaymentTerms(PaymentTermId)
);
GO
...
CREATE TABLE CustomerContacts (
    ...
    UpdatedAt       datetime2(0) NULL,
    UpdatedBy       int NULL,
    CONSTRAINT FK_CustomerContacts_Customers FOREIGN KEY (CustomerId) REFERENCES Customers(CustomerId)
);
GO
...
CREATE TABLE Suppliers (
    ...
    CONSTRAINT FK_Suppliers_PaymentTerms FOREIGN KEY (PaymentTermId) REFERENCES PaymentTerms(PaymentTermId)
);
GO
...
CREATE TABLE Employees (
    ...
    CONSTRAINT FK_Employees_JobTitles   FOREIGN KEY (JobTitleId)   REFERENCES JobTitles(JobTitleId),
    CONSTRAINT FK_Employees_Branches    FOREIGN KEY (BranchId)     REFERENCES Branches(BranchId),
    CONSTRAINT FK_Employees_Manager     FOREIGN KEY (ManagerEmployeeId) REFERENCES Employees(EmployeeId)
);
GO
...
CREATE TABLE Drivers (
    ...
    CONSTRAINT FK_Drivers_Employees FOREIGN KEY (EmployeeId) REFERENCES Employees(EmployeeId),
    CONSTRAINT FK_Drivers_Suppliers FOREIGN KEY (SupplierId) REFERENCES Suppliers(SupplierId)
);
GO
...
CREATE TABLE Trailers (
    ...
    CONSTRAINT FK_Trailers_Suppliers FOREIGN KEY (SupplierId) REFERENCES Suppliers(SupplierId)
);
GO
...
CREATE TABLE VehicleMaintenance (
    ...
    CONSTRAINT FK_VehMaint_Vehicles  FOREIGN KEY (VehicleId)  REFERENCES Vehicles(VehicleId),
    CONSTRAINT FK_VehMaint_Branches  FOREIGN KEY (BranchId)   REFERENCES Branches(BranchId),
    CONSTRAINT FK_VehMaint_Suppliers FOREIGN KEY (SupplierId) REFERENCES Suppliers(SupplierId)
);
GO
...
CREATE TABLE CustomerPriceRules (
    ...
    CONSTRAINT FK_CPR_Customers      FOREIGN KEY (CustomerId)      REFERENCES Customers(CustomerId),
    CONSTRAINT FK_CPR_PriceLists     FOREIGN KEY (PriceListId)     REFERENCES PriceLists(PriceListId),
    CONSTRAINT FK_CPR_Services       FOREIGN KEY (ServiceId)       REFERENCES Services(ServiceId),
    CONSTRAINT FK_CPR_Ports          FOREIGN KEY (PortId)          REFERENCES Ports(PortId),
    CONSTRAINT FK_CPR_Destinations   FOREIGN KEY (DestinationId)   REFERENCES Destinations(DestinationId),
    CONSTRAINT FK_CPR_ContainerTypes FOREIGN KEY (ContainerTypeId) REFERENCES ContainerTypes(ContainerTypeId),
    CONSTRAINT FK_CPR_TripTypes      FOREIGN KEY (TripTypeId)      REFERENCES TripTypes(TripTypeId),
    CONSTRAINT FK_CPR_TaxRates       FOREIGN KEY (TaxRateId)       REFERENCES TaxRates(TaxRateId)
);
GO

-- B12: منع تداخل قاعدتين بنفس الأبعاد والفترتين (يسبب غموض في السعر)
CREATE UNIQUE INDEX UX_CustomerPriceRules_NoOverlap
    ON CustomerPriceRules (CustomerId, ServiceId, PortId, DestinationId, ContainerTypeId, TripTypeId, ValidFrom)
...
CREATE TABLE Bookings (
    ...
    CONSTRAINT FK_Bookings_Ports        FOREIGN KEY (PortId)        REFERENCES Ports(PortId),
    CONSTRAINT FK_Bookings_Destinations FOREIGN KEY (DestinationId) REFERENCES Destinations(DestinationId),
    CONSTRAINT FK_Bookings_TripTypes    FOREIGN KEY (TripTypeId)    REFERENCES TripTypes(TripTypeId),
    CONSTRAINT FK_Bookings_Contacts     FOREIGN KEY (ContactId)     REFERENCES CustomerContacts(CustomerContactId)
);
GO
...
CREATE TABLE Containers (
    ...
    CONSTRAINT FK_Containers_Types  FOREIGN KEY (ContainerTypeId) REFERENCES ContainerTypes(ContainerTypeId)
);
GO
...
CREATE TABLE ContainerMovements (
    ...
    CONSTRAINT FK_ContainerMovements_Containers FOREIGN KEY (ContainerId) REFERENCES Containers(ContainerId),
    CONSTRAINT FK_ContainerMovements_Ports      FOREIGN KEY (PortId)      REFERENCES Ports(PortId)
);
GO
...
CREATE TABLE Operations (
    ...
    CONSTRAINT FK_Operations_Branches     FOREIGN KEY (BranchId)      REFERENCES Branches(BranchId),
    CONSTRAINT FK_Operations_Bookings     FOREIGN KEY (BookingId)     REFERENCES Bookings(BookingId),
    CONSTRAINT FK_Operations_Customers    FOREIGN KEY (CustomerId)    REFERENCES Customers(CustomerId),
    CONSTRAINT FK_Operations_Services     FOREIGN KEY (ServiceId)     REFERENCES Services(ServiceId),
    CONSTRAINT FK_Operations_Ports        FOREIGN KEY (PortId)        REFERENCES Ports(PortId),
    CONSTRAINT FK_Operations_Destinations FOREIGN KEY (DestinationId) REFERENCES Destinations(DestinationId),
    CONSTRAINT FK_Operations_TripTypes    FOREIGN KEY (TripTypeId)    REFERENCES TripTypes(TripTypeId)
);
GO
...
CREATE TABLE Trips (
    ...
    CONSTRAINT FK_Trips_Branches  FOREIGN KEY (BranchId)   REFERENCES Branches(BranchId),
    ...
    CONSTRAINT FK_Trips_Trailers  FOREIGN KEY (TrailerId)  REFERENCES Trailers(TrailerId),
    CONSTRAINT FK_Trips_TripTypes FOREIGN KEY (TripTypeId) REFERENCES TripTypes(TripTypeId)
);
GO

-- 🔴 A2: ده الجدول اللي بيخلّي التجميع ممكن — رحلة ← كذا عملية
CREATE TABLE TripOperations (
    ...
    CONSTRAINT FK_TripOperations_Trip      FOREIGN KEY (TripId)      REFERENCES Trips(TripId),
    CONSTRAINT FK_TripOperations_Operation FOREIGN KEY (OperationId) REFERENCES Operations(OperationId)
);
GO
...
CREATE TABLE TripCostAllocations (
    ...
    CONSTRAINT FK_TCA_Trip      FOREIGN KEY (TripId)      REFERENCES Trips(TripId),
    CONSTRAINT FK_TCA_Operation FOREIGN KEY (OperationId) REFERENCES Operations(OperationId)
);
GO
...
CREATE TABLE OperationRevenueItems (
    ...
    --    في SQL Server ماينفعش يشير لجدول تاني (TaxRates).
    ...
    CreatedBy       int NULL,
    CONSTRAINT FK_ORI_Operations FOREIGN KEY (OperationId) REFERENCES Operations(OperationId),
    CONSTRAINT FK_ORI_Services   FOREIGN KEY (ServiceId)   REFERENCES Services(ServiceId),
    CONSTRAINT FK_ORI_PriceRules FOREIGN KEY (PriceRuleId) REFERENCES CustomerPriceRules(CustomerPriceRuleId),
    CONSTRAINT FK_ORI_TaxRates   FOREIGN KEY (TaxRateId)   REFERENCES TaxRates(TaxRateId)
);
GO
...
CREATE TABLE DriverCustodies (
    ...
    CONSTRAINT FK_Custodies_Branches FOREIGN KEY (BranchId) REFERENCES Branches(BranchId),
    CONSTRAINT FK_Custodies_Trips    FOREIGN KEY (TripId)   REFERENCES Trips(TripId)
);
GO

CREATE TABLE CustodyTransactions (
    ...
    CONSTRAINT FK_CustodyTx_Custody FOREIGN KEY (CustodyId) REFERENCES DriverCustodies(CustodyId)
);
GO
...
CREATE TABLE Expenses (
    ...
    CONSTRAINT FK_Expenses_Operations FOREIGN KEY (OperationId)  REFERENCES Operations(OperationId),
    CONSTRAINT FK_Expenses_Trips     FOREIGN KEY (TripId)        REFERENCES Trips(TripId),
    CONSTRAINT FK_Expenses_Custodies FOREIGN KEY (CustodyId)     REFERENCES DriverCustodies(CustodyId),
    CONSTRAINT FK_Expenses_Drivers   FOREIGN KEY (DriverId)      REFERENCES Drivers(DriverId),
    CONSTRAINT FK_Expenses_Suppliers FOREIGN KEY (SupplierId)    REFERENCES Suppliers(SupplierId),
    CONSTRAINT FK_Expenses_Types     FOREIGN KEY (ExpenseTypeId) REFERENCES ExpenseTypes(ExpenseTypeId),
    CONSTRAINT FK_Expenses_TaxRates  FOREIGN KEY (TaxRateId)     REFERENCES TaxRates(TaxRateId)
);
GO

ALTER TABLE CustodyTransactions
    ADD CONSTRAINT FK_CustodyTx_Expense FOREIGN KEY (ExpenseId) REFERENCES Expenses(ExpenseId);
GO
...
CREATE TABLE Invoices (
    ...
    CONSTRAINT FK_Invoices_Branches  FOREIGN KEY (BranchId)         REFERENCES Branches(BranchId),
    CONSTRAINT FK_Invoices_Customers FOREIGN KEY (CustomerId)       REFERENCES Customers(CustomerId),
    CONSTRAINT FK_Invoices_Original  FOREIGN KEY (OriginalInvoiceId) REFERENCES Invoices(InvoiceId)
);
GO
...
CREATE TABLE InvoiceItems (
    ...
    CONSTRAINT FK_InvoiceItems_Operations FOREIGN KEY (OperationId) REFERENCES Operations(OperationId),
    CONSTRAINT FK_InvoiceItems_Services   FOREIGN KEY (ServiceId)   REFERENCES Services(ServiceId),
    CONSTRAINT FK_InvoiceItems_PriceRules FOREIGN KEY (PriceRuleId) REFERENCES CustomerPriceRules(CustomerPriceRuleId),
    CONSTRAINT FK_InvoiceItems_TaxRates   FOREIGN KEY (TaxRateId)   REFERENCES TaxRates(TaxRateId)
);
GO

CREATE TABLE InvoiceOperations (
    ...
    CONSTRAINT FK_InvoiceOperations_Invoices   FOREIGN KEY (InvoiceId)   REFERENCES Invoices(InvoiceId),
    CONSTRAINT FK_InvoiceOperations_Operations FOREIGN KEY (OperationId) REFERENCES Operations(OperationId)
);
GO
...
CREATE TABLE Payments (
    ...
    CONSTRAINT FK_Payments_Branches  FOREIGN KEY (BranchId)        REFERENCES Branches(BranchId),
    ...
    CONSTRAINT FK_Payments_Reversed  FOREIGN KEY (ReversedByPaymentId) REFERENCES Payments(PaymentId)
);
GO
...
CREATE TABLE PaymentAllocations (
    ...
    CONSTRAINT FK_PaymentAlloc_Payments FOREIGN KEY (PaymentId) REFERENCES Payments(PaymentId),
    CONSTRAINT FK_PaymentAlloc_Invoices FOREIGN KEY (InvoiceId) REFERENCES Invoices(InvoiceId)
);
GO

-- B3: الـ Prompt قسم 29 — Sent Date / Sent By / Recipient / Send Status / Error Message
CREATE TABLE InvoiceSendLogs (
    ...
    CONSTRAINT FK_InvoiceSendLogs_Invoices FOREIGN KEY (InvoiceId) REFERENCES Invoices(InvoiceId)
);
GO
...
CREATE TABLE SupplierInvoices (
    ...
    CONSTRAINT FK_SupplierInvoices_Branches  FOREIGN KEY (BranchId)   REFERENCES Branches(BranchId),
...
CREATE TABLE SupplierInvoiceItems (
    ...
    CONSTRAINT FK_SII_Invoices   FOREIGN KEY (SupplierInvoiceId) REFERENCES SupplierInvoices(SupplierInvoiceId),
    CONSTRAINT FK_SII_Trips      FOREIGN KEY (TripId)            REFERENCES Trips(TripId),
    CONSTRAINT FK_SII_Operations FOREIGN KEY (OperationId)       REFERENCES Operations(OperationId),
    CONSTRAINT FK_SII_Types      FOREIGN KEY (ExpenseTypeId)     REFERENCES ExpenseTypes(ExpenseTypeId)
);
GO
...
CREATE TABLE SupplierPayments (
    ...
    CONSTRAINT FK_SupplierPayments_Branches  FOREIGN KEY (BranchId)        REFERENCES Branches(BranchId),
    CONSTRAINT FK_SupplierPayments_Suppliers FOREIGN KEY (SupplierId)      REFERENCES Suppliers(SupplierId),
    CONSTRAINT FK_SupplierPayments_Methods   FOREIGN KEY (PaymentMethodId) REFERENCES PaymentMethods(PaymentMethodId)
);
GO

CREATE TABLE SupplierInvoiceAllocations (
    ...
    CONSTRAINT FK_SIA_Payments FOREIGN KEY (SupplierPaymentId) REFERENCES SupplierPayments(SupplierPaymentId),
    CONSTRAINT FK_SIA_Invoices FOREIGN KEY (SupplierInvoiceId) REFERENCES SupplierInvoices(SupplierInvoiceId)
);
GO
...
CREATE TABLE CashBoxes (
    ...
    CONSTRAINT FK_CashBoxes_Branches FOREIGN KEY (BranchId) REFERENCES Branches(BranchId)
);
GO
...
CREATE TABLE CashTransactions (
    ...
    CONSTRAINT FK_CashTx_Branches     FOREIGN KEY (BranchId)        REFERENCES Branches(BranchId),
    CONSTRAINT FK_CashTx_CashBoxes    FOREIGN KEY (CashBoxId)       REFERENCES CashBoxes(CashBoxId),
    CONSTRAINT FK_CashTx_Methods      FOREIGN KEY (PaymentMethodId) REFERENCES PaymentMethods(PaymentMethodId),
    CONSTRAINT FK_CashTx_Customers    FOREIGN KEY (CustomerId)      REFERENCES Customers(CustomerId),
    CONSTRAINT FK_CashTx_Suppliers    FOREIGN KEY (SupplierId)      REFERENCES Suppliers(SupplierId),
    CONSTRAINT FK_CashTx_Operations   FOREIGN KEY (OperationId)     REFERENCES Operations(OperationId),
    CONSTRAINT FK_CashTx_Invoices     FOREIGN KEY (InvoiceId)       REFERENCES Invoices(InvoiceId),
    CONSTRAINT FK_CashTx_Custodies    FOREIGN KEY (CustodyId)       REFERENCES DriverCustodies(CustodyId),
    CONSTRAINT FK_CashTx_Counterpart  FOREIGN KEY (CounterpartCashBoxId) REFERENCES CashBoxes(CashBoxId),
    CONSTRAINT FK_CashTx_Reversed     FOREIGN KEY (ReversedByTransactionId) REFERENCES CashTransactions(CashTransactionId)
);
GO

-- الـ FK اللي اتأجل
ALTER TABLE Payments
    ADD CONSTRAINT FK_Payments_CashTx FOREIGN KEY (CashTransactionId) REFERENCES CashTransactions(CashTransactionId);
ALTER TABLE SupplierPayments
    ADD CONSTRAINT FK_SupplierPayments_CashTx FOREIGN KEY (CashTransactionId) REFERENCES CashTransactions(CashTransactionId);
GO
...
CREATE TABLE Documents (
    ...
    CONSTRAINT FK_Documents_Types    FOREIGN KEY (DocumentTypeId) REFERENCES DocumentTypes(DocumentTypeId),
    CONSTRAINT FK_Documents_Branches FOREIGN KEY (BranchId)       REFERENCES Branches(BranchId)
);
GO
...
CREATE TABLE Notifications (
    ...
    CONSTRAINT FK_Notifications_Branches FOREIGN KEY (BranchId) REFERENCES Branches(BranchId)
);
GO
...
CREATE TABLE CustomerPortalTokens (
    ...
    CONSTRAINT FK_PortalTokens_Customers FOREIGN KEY (CustomerId) REFERENCES Customers(CustomerId),
    CONSTRAINT FK_PortalTokens_Invoices  FOREIGN KEY (InvoiceId)  REFERENCES Invoices(InvoiceId)
);
GO

CREATE TABLE PortalAccessLogs (
    ...
    CONSTRAINT FK_PortalAccess_Customers FOREIGN KEY (CustomerId)    REFERENCES Customers(CustomerId),
    CONSTRAINT FK_PortalAccess_Tokens    FOREIGN KEY (PortalTokenId) REFERENCES CustomerPortalTokens(PortalTokenId)
);
GO
...
CREATE UNIQUE INDEX UX_AspNetRoles_Code ON AspNetRoles (RoleCode);
GO
...
CREATE TABLE AspNetUsers (
    ...
    CONSTRAINT FK_AspNetUsers_Employees FOREIGN KEY (EmployeeId) REFERENCES Employees(EmployeeId),
    CONSTRAINT FK_AspNetUsers_Branches  FOREIGN KEY (BranchId)   REFERENCES Branches(BranchId)
);
...
ALTER TABLE Notifications
    ADD CONSTRAINT FK_Notifications_Users FOREIGN KEY (UserId) REFERENCES AspNetUsers(Id);
GO
...
CREATE TABLE AppRolePermissions (
    ...
    CONSTRAINT FK_ARP_Roles       FOREIGN KEY (RoleId)       REFERENCES AspNetRoles(Id),
    CONSTRAINT FK_ARP_Permissions FOREIGN KEY (PermissionId) REFERENCES AppPermissions(PermissionId)
);
GO
...
CREATE TABLE AppUserPermissions (
    ...
    CONSTRAINT FK_AUP_Users       FOREIGN KEY (UserId)       REFERENCES AspNetUsers(Id),
    CONSTRAINT FK_AUP_Permissions FOREIGN KEY (PermissionId) REFERENCES AppPermissions(PermissionId)
);
GO
...
CREATE TABLE NumberSequences (
    ...
    CONSTRAINT FK_NumberSequences_Branches FOREIGN KEY (BranchId) REFERENCES Branches(BranchId)
);
GO

CREATE UNIQUE INDEX UX_NumberSequences_Branch
    ON NumberSequences (BranchId, DocumentType, Year)
    WHERE BranchId IS NOT NULL;

CREATE UNIQUE INDEX UX_NumberSequences_Global
    ON NumberSequences (DocumentType, Year)
...
-- Bookings
CREATE INDEX IX_Bookings_Customer_Status ON Bookings (CustomerId, Status, BookingDate) WHERE IsDeleted = 0;
CREATE INDEX IX_Bookings_Port_Date       ON Bookings (PortId, RequestedDate) WHERE IsDeleted = 0;
CREATE INDEX IX_Bookings_Branch_Status   ON Bookings (BranchId, Status) WHERE IsDeleted = 0;

-- Containers
CREATE INDEX IX_Containers_Status ON Containers (CurrentStatus) WHERE IsDeleted = 0;
CREATE INDEX IX_ContainerMovements_Container ON ContainerMovements (ContainerId, MovementDate DESC);
...
-- Operations
CREATE INDEX IX_Operations_Customer_Status ON Operations (CustomerId, Status, PlannedDate) WHERE IsDeleted = 0;
CREATE INDEX IX_Operations_Booking  ON Operations (BookingId);
CREATE INDEX IX_Operations_Branch_Status ON Operations (BranchId, Status) WHERE IsDeleted = 0;
...
CREATE INDEX IX_OperationRevenueItems_Operation ON OperationRevenueItems (OperationId);

-- 🔴 Trips — أهم الفهارس (التجميع + تعارض الموارد)
CREATE INDEX IX_Trips_Driver_Date  ON Trips (DriverId, PlannedStartAt)  WHERE IsDeleted = 0;
CREATE INDEX IX_Trips_Vehicle_Date ON Trips (VehicleId, PlannedStartAt) WHERE IsDeleted = 0;
CREATE INDEX IX_Trips_Trailer_Date ON Trips (TrailerId, PlannedStartAt) WHERE IsDeleted = 0;
CREATE INDEX IX_Trips_Branch_Status ON Trips (BranchId, Status) WHERE IsDeleted = 0;
CREATE INDEX IX_TripOperations_Operation ON TripOperations (OperationId);

-- Custody
CREATE INDEX IX_Custodies_Owner_Status ON DriverCustodies (OwnerType, OwnerId, Status) WHERE IsDeleted = 0;
CREATE INDEX IX_Custodies_Trip ON DriverCustodies (TripId);
CREATE INDEX IX_CustodyTransactions_Custody ON CustodyTransactions (CustodyId, TransactionType);

-- Expenses
CREATE INDEX IX_Expenses_Operation ON Expenses (OperationId, ExpenseDate) INCLUDE (Amount, Status, ExpenseTypeId) WHERE IsDeleted = 0;
CREATE INDEX IX_Expenses_Trip      ON Expenses (TripId) INCLUDE (Amount, Status) WHERE IsDeleted = 0;
CREATE INDEX IX_Expenses_Custody   ON Expenses (CustodyId) WHERE IsDeleted = 0;
CREATE INDEX IX_Expenses_Driver    ON Expenses (DriverId, ExpenseDate) WHERE IsDeleted = 0;
CREATE INDEX IX_Expenses_Unpaid    ON Expenses (PaymentStatus, ExpenseDate) WHERE IsDeleted = 0;

-- Invoices
CREATE INDEX IX_Invoices_Customer_Date ON Invoices (CustomerId, InvoiceDate, Status) WHERE IsDeleted = 0;
CREATE INDEX IX_Invoices_Branch_Status ON Invoices (BranchId, Status, PaymentStatus) WHERE IsDeleted = 0;
CREATE INDEX IX_InvoiceItems_Operation ON InvoiceItems (OperationId);
CREATE INDEX IX_InvoiceOperations_Operation ON InvoiceOperations (OperationId);
CREATE INDEX IX_InvoiceSendLogs_Invoice ON InvoiceSendLogs (InvoiceId, SentAt DESC);

-- Payments
CREATE INDEX IX_Payments_Customer ON Payments (CustomerId, PaymentDate) WHERE IsDeleted = 0;
CREATE INDEX IX_PaymentAllocations_Invoice ON PaymentAllocations (InvoiceId);

-- Supplier payables
CREATE INDEX IX_SupplierInvoices_Supplier ON SupplierInvoices (SupplierId, InvoiceDate, Status) WHERE IsDeleted = 0;
CREATE INDEX IX_SupplierInvoiceItems_Trip ON SupplierInvoiceItems (TripId);
CREATE INDEX IX_SupplierPayments_Supplier ON SupplierPayments (SupplierId, PaymentDate) WHERE IsDeleted = 0;

-- Treasury
CREATE INDEX IX_CashTransactions_Box_Date ON CashTransactions (CashBoxId, TransactionDate) WHERE IsDeleted = 0;
CREATE INDEX IX_CashTransactions_Customer ON CashTransactions (CustomerId, TransactionType, TransactionDate) WHERE IsDeleted = 0;

-- Documents / Notifications
CREATE INDEX IX_Documents_Entity ON Documents (EntityType, EntityId) WHERE IsDeleted = 0;
CREATE INDEX IX_Documents_Expiry ON Documents (ExpiryDate) WHERE IsDeleted = 0 AND ExpiryDate IS NOT NULL;
...
CREATE INDEX IX_CustomerPriceRules_Lookup
    ON CustomerPriceRules (CustomerId, ServiceId, PortId, ContainerTypeId, ValidFrom, ValidTo)
    WHERE IsDeleted = 0 AND IsActive = 1;
...
-- Portal
CREATE INDEX IX_PortalAccessLogs_Customer ON PortalAccessLogs (CustomerId, AccessedAt DESC);
GO
...
CREATE OR ALTER TRIGGER trg_CustodyTransactions_Sync ON CustodyTransactions
...
CREATE OR ALTER TRIGGER trg_OperationRevenueItems_Sync ON OperationRevenueItems
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER TRIGGER trg_Operations_CostSync ON TripCostAllocations
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    OUTER APPLY (
        ...
        FROM TripCostAllocations
        WHERE OperationId = o.OperationId
    ) a;
END
GO

CREATE OR ALTER TRIGGER trg_Expenses_OpCostSync ON Expenses
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER TRIGGER trg_PaymentAllocations_InvoiceSync ON PaymentAllocations
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    OUTER APPLY (
        ...
        INNER JOIN Payments py ON py.PaymentId = pa.PaymentId
...
CREATE OR ALTER TRIGGER trg_SupplierInvoiceAllocations_Sync ON SupplierInvoiceAllocations
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
    ...
    OUTER APPLY (
        ...
        FROM SupplierInvoiceAllocations sia
        INNER JOIN SupplierPayments sp ON sp.SupplierPaymentId = sia.SupplierPaymentId
...
CREATE OR ALTER TRIGGER trg_CashTransactions_BalanceSync ON CashTransactions
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
    SET NOCOUNT ON;
...
IF NOT EXISTS (SELECT 1 FROM Branches)
    INSERT Branches (BranchCode, NameAr, NameEn)
    VALUES ('MAIN', N'الفرع الرئيسي', N'Main Branch');
GO

IF NOT EXISTS (SELECT 1 FROM Departments)
    INSERT Departments (Code, NameAr, NameEn) VALUES
...
IF NOT EXISTS (SELECT 1 FROM JobTitles)
    INSERT JobTitles (Code, NameAr, NameEn) VALUES
...
IF NOT EXISTS (SELECT 1 FROM PaymentTerms)
    INSERT PaymentTerms (Code, NameAr, NameEn, DueDays) VALUES
...
IF NOT EXISTS (SELECT 1 FROM PaymentMethods)
    INSERT PaymentMethods (Code, NameAr, NameEn, RequiresChequeNo, IsCashBased, SortOrder) VALUES
...
IF NOT EXISTS (SELECT 1 FROM TaxRates)
    INSERT TaxRates (Code, NameAr, NameEn, Rate, TaxKind, IsDeductible, RequiresExemptionReason, ValidFrom) VALUES
...
IF NOT EXISTS (SELECT 1 FROM Ports)
    INSERT Ports (PortCode, NameAr, NameEn, CityAr, CityEn) VALUES
...
IF NOT EXISTS (SELECT 1 FROM Destinations)
    INSERT Destinations (Code, NameAr, NameEn, CityAr, CityEn) VALUES
...
IF NOT EXISTS (SELECT 1 FROM TripTypes)
BEGIN
    DECLARE @trStd int = (SELECT TaxRateId FROM TaxRates WHERE Code = 'STD14');
    DECLARE @trSch int = (SELECT TaxRateId FROM TaxRates WHERE Code = 'SCH5');
    DECLARE @trExm int = (SELECT TaxRateId FROM TaxRates WHERE Code = 'EXEMPT');
    INSERT TripTypes (Code, NameAr, NameEn, DefaultTaxRateId) VALUES
...
BEGIN
    DECLARE @svcTax int = (SELECT TaxRateId FROM TaxRates WHERE Code = 'SCH5');
    -- ⚠️ الأسعار الافتراضية = 0 → لازم تتعبّى من الشاشة قبل التشغيل
    INSERT Services (ServiceCode, NameAr, NameEn, Unit, DefaultSellingPrice, DefaultCost, TaxRateId, IsTaxable) VALUES
...
IF NOT EXISTS (SELECT 1 FROM ContainerTypes)
    INSERT ContainerTypes (Code, NameAr, NameEn, SizeFeet, IsReefer) VALUES
...
IF NOT EXISTS (SELECT 1 FROM ExpenseTypes)
    INSERT ExpenseTypes (Code, NameAr, NameEn, IsCustodyAllowed, IsOperationCost, IsTaxDeductible) VALUES
...
IF NOT EXISTS (SELECT 1 FROM DocumentTypes)
    INSERT DocumentTypes (Code, NameAr, NameEn, HasExpiry) VALUES
...
IF NOT EXISTS (SELECT 1 FROM OperationStatuses)
    INSERT OperationStatuses (Code, NameAr, NameEn, SortOrder, IsTerminal, ColorHex) VALUES
...
IF NOT EXISTS (SELECT 1 FROM CompanyProfile)
    ...
    VALUES
        (1,
         ...
         (SELECT TaxRateId FROM TaxRates WHERE Code = 'SCH5'));
GO

IF NOT EXISTS (SELECT 1 FROM CashBoxes)
BEGIN
    DECLARE @cbBranch int = (SELECT TOP 1 BranchId FROM Branches ORDER BY BranchId);
    INSERT CashBoxes (BranchId, Code, NameAr, NameEn) VALUES
...
IF NOT EXISTS (SELECT 1 FROM NumberSequences)
BEGIN
    DECLARE @yr smallint = YEAR(SYSUTCDATETIME());
    INSERT NumberSequences (BranchId, DocumentType, Prefix, Year, CurrentNumber, NumberLength, ResetPeriod) VALUES
...
IF NOT EXISTS (SELECT 1 FROM AppPermissions)
BEGIN
    SET IDENTITY_INSERT AppPermissions ON;
    INSERT AppPermissions (PermissionId, PermissionCode, NameAr, NameEn, ModuleCode, ActionCode, SortOrder) VALUES
    -- DASHBOARD
...
IF NOT EXISTS (SELECT 1 FROM AspNetRoles)
    INSERT AspNetRoles (Name, NormalizedName, RoleCode, NameAr, NameEn, IsSystem) VALUES
...
IF NOT EXISTS (SELECT 1 FROM AppRolePermissions)
BEGIN
    -- ADMIN: كل الصلاحيات
    INSERT AppRolePermissions (RoleId, PermissionId)
    SELECT r.Id, p.PermissionId FROM AspNetRoles r CROSS JOIN AppPermissions p
...
FROM Operations o
INNER JOIN Bookings  b ON b.BookingId  = o.BookingId
...
OUTER APPLY (
    ...
    FROM vw_CustomerInvoiceBalance ib
...
LEFT JOIN Trailers tr ON tr.TrailerId = t.TrailerId
OUTER APPLY (
    SELECT COUNT(*) AS OperationCount, COUNT(DISTINCT opx.CustomerId) AS CustomerCount
    FROM TripOperations tpo
    INNER JOIN Operations opx ON opx.OperationId = tpo.OperationId
...
OUTER APPLY (
    ...
    FROM TripCostAllocations tca WHERE tca.TripId = t.TripId
) al
OUTER APPLY (
    SELECT SUM(dc.AmountIssued) AS IssuedTotal, SUM(dc.AmountSpent) AS SpentTotal, SUM(dc.AmountReturned) AS ReturnedTotal
    FROM DriverCustodies dc WHERE dc.TripId = t.TripId AND dc.IsDeleted = 0
) cu
WHERE t.IsDeleted = 0;
GO
...
FROM Vehicles v
OUTER APPLY (
    SELECT TOP 1 m.NextDueDate FROM VehicleMaintenance m

...
🔴 **وباقي محاولتين بس!** (محاولة غلط تانية الساعة 22:37 — يا إما انت يا إما حد فاتح الصفحة. لو القفل اشتغل استنى 15 دقيقة).
السيرفر شغل بالنسخة الجديدة: `https://localhost:7001` + `http://localhost:5001` (بروفايل `FastCom.Server`).
🔴 **فخ البورت:** `dotnet run --project src\FastCom.Server --launch-profile https` ← الاسم غلط (مفيش بروفايل `https`) فيقع صامت على بورت 5000! شغّله **من غير** `--launch-profile` أصلًا (بياخد أول بروفايل تلقائيًا).
